...
impl BatchState {
    ...
    pub fn transition_to_written(&self) -> Option<()> {
        // println!("{} state transition to 1", self.id);
        match self
            .state
            .compare_exchange_weak(0, 1, Ordering::AcqRel, Ordering::Acquire)
        {
            Ok(_) => {
                self.ready_for_execute_notifier.notify_waiters();
                Some(())
            }
            Err(state) => {
    ...
    pub fn transition_to_executed(&self) -> Option<()> {
        // println!("{} state transition to 2", self.id);
        match self
            .state
            .compare_exchange_weak(1, 2, Ordering::AcqRel, Ordering::Acquire)
        {
            Ok(_) => {
                self.output_ready_notifier.notify_waiters();
                Some(())
            }
            Err(state) => {
    ...
    pub fn transition_to_ready_to_use(&self) -> Option<()> {
        // println!("{} state transition to 3", self.id);
        match self
            .state
            .compare_exchange_weak(2, 3, Ordering::AcqRel, Ordering::Acquire)
        {
            Ok(_) => Some(()),
            Err(_) => None,
        }
    ...
    pub fn is_ready_to_use_or_open(&self) -> bool {
    ...
    pub fn transition_to_open(&self) -> Option<()> {
        // println!("{} state transition to 0", self.id);
        if self.state.load(Ordering::SeqCst) == 0 {
            return Some(());
        }
        match self
            .state
            .compare_exchange_weak(3, 0, Ordering::AcqRel, Ordering::Acquire)
        {
            Ok(_) => Some(()),
            Err(state) => {
...
impl TensorBatch {
    pub fn close_for_write(&self) {
        loop {
            ...
            if !self.state.is_ready_to_use_or_open() {
                // println!("{} buffer already closed", self.id);
                return;
            }
            ...
            // // println!(
            //     "{} {written_slots} == min({reserved_slots}, {})",
            ...
            if written_slots == clamped_slots {
                ...
                match self.reserved_slots.compare_exchange_weak(
                    ...
                    Ok(_) => {
                        ...
                        let mut batch_complete = self.trace.batch_complete.borrow_mut();
                        *batch_complete = self.trace.batch_first_open.borrow().elapsed();
    ...
    pub fn new(id: usize, batch_size: usize, inputs: &Vec<&Input>) -> Result<TensorBatch, ()> {
        ...
        #[cfg(not(feature = "loom"))]
        {
            ...
            let trace = Trace {
                batch_first_open: RefCell::from(std::time::Instant::now()),
                batch_complete: RefCell::from(std::time::Duration::from_nanos(0)),
                batch_inference_start: RefCell::from(std::time::Duration::from_nanos(0)),
                batch_inference_done: RefCell::from(std::time::Duration::from_nanos(0)),
                batch_released: RefCell::from(std::time::Duration::from_nanos(0)),
            };
    ...
    pub fn append_on_slot<'a>(
        ...
        ring_buffer: &'a BatchRingBuffer,
    ) -> Result<WriteReservation<'a>, usize> {
    ...
    pub fn append(&self, data: &HashMap<String, DynTensor>) -> Result<usize, usize> {
        ...
        if slot == 0 {
            *self.trace.batch_first_open.borrow_mut() = std::time::Instant::now()
        }
        ...
        F: AsyncFnOnce(HashMap<String, ValueRef<'_, DynTensorValueType>>) -> BatchedOutputs,
    {
        ...
        *self.trace.batch_inference_start.borrow_mut() = self.trace.batch_first_open.borrow().elapsed();
        unsafe {
            let result = Some(f(input).await);
            let ptr = self.output_data.get();
            // println!("{} Executor: execute", self.id);
            *ptr = result;
        }
        fence(Ordering::Release);
        *self.trace.batch_inference_done.borrow_mut() = self.trace.batch_first_open.borrow().elapsed();
    ...
    pub fn reset(&self) {
        ...
        *self.trace.batch_released.borrow_mut() = self.trace.batch_first_open.borrow().elapsed();
        self.trace.print_debug();
        self.state.transition_to_open().unwrap();
    }

    pub fn is_ready_to_use_or_open(&self) -> bool {
        // the ring buffer can consume buffer so quickly that a buffer can
